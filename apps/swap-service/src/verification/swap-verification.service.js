...
var SwapVerificationService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.SwapVerificationService = void 0;
...
const thorchainToNativePrecision = (thorchainAmount, nativePrecision) => {
    const diff = nativePrecision - THORCHAIN_PRECISION;
    if (diff === 0)
        return thorchainAmount;
    if (diff > 0)
        return thorchainAmount + '0'.repeat(diff);
    const trimmed = thorchainAmount.slice(0, diff);
    return trimmed || '0';
};
let SwapVerificationService = SwapVerificationService_1 = class SwapVerificationService {
    constructor(httpService) {
        this.httpService = httpService;
        this.logger = new common_1.Logger(SwapVerificationService_1.name);
        this.oneClickServiceInitialized = false;
    }
    initializeOneClickService(apiKey) {
        if (this.oneClickServiceInitialized)
            return;
        const oneClickBaseUrl = 'https://1click.chaindefuser.com';
        one_click_sdk_typescript_1.OpenAPI.BASE = oneClickBaseUrl;
        one_click_sdk_typescript_1.OpenAPI.TOKEN = apiKey;
        this.oneClickServiceInitialized = true;
        this.logger.log('OneClickService initialized');
    }
    async verifySwapAffiliate(swapId, protocol, sellChainId, txHash, metadata) {
        try {
    ...
    async verifyNearIntents(swapId, metadata) {
        ...
        try {
            ...
            const swapDetails = statusResponse.swapDetails;
            const quoteAmounts = statusResponse.quoteResponse?.quote;
            let verifiedSellAmountCryptoBaseUnit;
            const rawDepositedAmount = swapDetails?.depositedAmount ??
                swapDetails?.amountIn ??
                quoteAmounts?.amountIn;
            if (rawDepositedAmount) {
                const sellAssetPrecision = metadata?.sellAssetPrecision;
    ...
    async verifyRelay(swapId, txHash) {
        if (!txHash) {
            return {
                isVerified: false,
                hasAffiliate: false,
        ...
        try {
            ...
            const verifiedSellAmountCryptoBaseUnit = request.data?.inTxs?.[0]?.data?.value?.toString() ??
    ...
    async verifyCowSwap(swapId, sellChainId, txHash, metadata) {
        ...
        try {
            ...
            const cowNetwork = (0, swapper_1.assertGetCowNetwork)(sellChainId);
            const response = await (0, rxjs_1.firstValueFrom)(this.httpService.get(`${cowswapApiUrl}/${cowNetwork}/api/v1/app_data/${appDataHash}`));
            ...
            let verifiedSellAmountCryptoBaseUnit;
            const orderUid = txHash || metadata?.cowswapOrderUid;
            if (orderUid) {
                try {
                    const orderResponse = await (0, rxjs_1.firstValueFrom)(this.httpService.get(`${cowswapApiUrl}/${cowNetwork}/api/v1/orders/${orderUid}`));
                    verifiedSellAmountCryptoBaseUnit =
            ...
            this.logger.log(`CowSwap verification for swap ${swapId}: appCode=${appCode}, hasPartnerFee=${!!partnerFee}, bps=${affiliateBps}, verified=${hasShapeshiftAffiliate}`);
            return {
                isVerified: true,
                hasAffiliate: hasShapeshiftAffiliate,
    ...
    async verifyPortals(swapId, sellChainId, metadata) {
        ...
        let expectedTreasuryAddress;
        try {
            expectedTreasuryAddress = (0, swapper_1.getTreasuryAddressFromChainId)(sellChainId);
        }
        catch {
        ...
        try {
            ...
            const portalsProxyUrl = process.env.PORTALS_PROXY_URL ||
                'https://api.proxy.shapeshift.com/api/v1/portals';
            const response = await (0, rxjs_1.firstValueFrom)(this.httpService.get(`${portalsProxyUrl}/v2/portal/status?orderId=${orderId}`));
            ...
            this.logger.log(`Portals verification for swap ${swapId}: partner=${partner}, expectedTreasury=${expectedTreasuryAddress}, verified=${hasShapeshiftAffiliate}, feeAmount=${feeAmount}`);
            return {
                isVerified: true,
                hasAffiliate: hasShapeshiftAffiliate,
    ...
    async verifyThorchain(swapId, txHash, metadata) {
        if (!txHash) {
            return {
                isVerified: false,
                hasAffiliate: false,
        ...
        try {
            ...
            const nodeUrl = process.env.VITE_THORCHAIN_NODE_URL ||
                'https://thornode.ninerealms.com';
            const txUrl = `${nodeUrl}/thorchain/tx/${txHash}`;
            this.logger.log(`Thorchain - Fetching tx from node API: ${txUrl}`);
            const response = await (0, rxjs_1.firstValueFrom)(this.httpService.get(txUrl));
            ...
            const verifiedSellAmountCryptoBaseUnit = firstCoinAmount
                ...
                : undefined;
            this.logger.log(`Thorchain verification for swap ${swapId}: memo=${memo}, affiliate=${shapeshiftAffiliate}, hasAffiliate=${hasShapeshiftAffiliate}, bps=${affiliateBps}`);
            return {
                isVerified: true,
                hasAffiliate: hasShapeshiftAffiliate,
                affiliateBps: hasShapeshiftAffiliate && affiliateBps ? affiliateBps : undefined,
    ...
    async verifyMaya(swapId, txHash, metadata) {
        if (!txHash) {
            return {
                isVerified: false,
                hasAffiliate: false,
        ...
        try {
            ...
            const nodeUrl = process.env.VITE_MAYACHAIN_NODE_URL ||
            ...
            const verifiedSellAmountCryptoBaseUnit = firstCoinAmount
                ? thorchainToNativePrecision(firstCoinAmount, sellAssetPrecision)
                : undefined;
            this.logger.log(`Maya verification for swap ${swapId}: memo=${memo}, affiliate=${shapeshiftAffiliate}, hasAffiliate=${hasShapeshiftAffiliate}, bps=${affiliateBps}`);
            return {
                isVerified: true,
                hasAffiliate: hasShapeshiftAffiliate,
                affiliateBps: hasShapeshiftAffiliate && affiliateBps ? affiliateBps : undefined,
    ...
    async verifyChainflip(swapId, metadata) {
        ...
        try {
            ...
            const response = await (0, rxjs_1.firstValueFrom)(this.httpService.get(statusUrl, { headers }));
            ...
            const verifiedSellAmountCryptoBaseUnit = (swapData.depositAmount ??
                swapData.ingressAmount ??
                swapData.sourceAmount)?.toString();
            return {
                isVerified: true,
                hasAffiliate: hasShapeshiftAffiliate,
    ...
    async verifyZrx(swapId, txHash, metadata) {
        const tradeHash = txHash ||
            metadata?.tradeHash ||
            metadata?.txHash;
        if (!tradeHash) {
            return {
        ...
        try {
            ...
            const zrxProxyUrl = process.env.ZRX_PROXY_URL ||
            ...
            const trade = trades.find((t) => t.txHash?.toLowerCase() === tradeHash.toLowerCase() ||
            ...
            const verifiedSellAmountCryptoBaseUnit = (trade.sellAmount ??
    ...
    async verifyBebop(swapId, txHash, metadata) {
        if (!txHash) {
            return {
                isVerified: false,
                hasAffiliate: false,
        ...
        try {
            ...
            const startNano = (swapTimestamp - oneHour) * 1000000; // Convert to nanoseconds
            const endNano = (swapTimestamp + oneHour) * 1000000;
            ...
            const response = await (0, rxjs_1.firstValueFrom)(this.httpService.get(requestUrl, { headers }));
            ...
            const trade = trades.find((t) => t.txHash?.toLowerCase() === txHash.toLowerCase());
            if (!trade) {
                return {
                    isVerified: false,
                    hasAffiliate: false,
...
exports.SwapVerificationService = SwapVerificationService = SwapVerificationService_1 = __decorate([
