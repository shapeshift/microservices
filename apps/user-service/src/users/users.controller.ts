...
export class UsersController {
  ...
  async addAccountIds(@Param('userId') userId: string, @Body() data: { accountIds: string[] }) {
    return this.usersService.addAccountIds(userId, data.accountIds)
  }

  @Post(':userId/account-id')
  async addAccountId(@Param('userId') userId: string, @Body() data: { accountId: string }) {
  ...
  @Post(':userId/devices')
  async registerDevice(@Param('userId') userId: string, @Body() data: { deviceToken: string; deviceType: DeviceType }) {
  ...
  async removeDevice(@Param('userId') userId: string, @Param('deviceId') deviceId: string) {
