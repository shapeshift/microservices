...
async function calculateReferralRewards(
  ...
  const totalVolume = Array.from(referralStats.values()).reduce((sum, stats) => sum + stats.totalVolumeUsd, 0)
  ...
  for (const [code, stats] of referralStats.entries()) {
    ...
    safeTxData.data.push(`0x${encodeDepositFunctionData(stats.ownerAddress, foxRewardWei.toString())}`)
